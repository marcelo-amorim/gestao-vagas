package br.com.marcelobrasil.gestao_vagas.modules.company.controllers;
...
import br.com.marcelobrasil.gestao_vagas.modules.company.useCases.CreateJobUseCase;
import br.com.marcelobrasil.gestao_vagas.modules.company.useCases.ListAllJobsByCompanyUseCase;
...
public class JobController {
    ...
    @Autowired private ListAllJobsByCompanyUseCase listAllJobsByCompanyUseCase;

    @PostMapping
    @PreAuthorize("hasRole('COMPANY')")
    @SecurityRequirement(name = "jwt_auth")
    @Tag(name = "Vagas", description = "Informações das vagas")
    @Operation(summary = "Cadastro de vagas", description = "Rota de cadastro de vagas")
    @ApiResponses({
        @ApiResponse(
                responseCode = "200",
    ...
    public ResponseEntity<Object> listByCompany(HttpServletRequest request) {
        var companyId = request.getAttribute("company_id");
        var result =
                this.listAllJobsByCompanyUseCase.execute(UUID.fromString(companyId.toString()));
        return ResponseEntity.ok().body(result);
    }
}
